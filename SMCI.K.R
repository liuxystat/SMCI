SMCI.K <-function(survfun=formula(data),curefun=formula(data),data=parent.frame(),
                  r=0,n.int=5,order=3,max.iter=1000,cov.rate=.001,kertype="epanechnikov",rescale=FALSE){
  call <- match.call()
  ...
  sdata$Zp <- Zp
  # Estimates
  temp.est<-EM.Ite(sdata=sdata,r=r,n.int=n.int,order=order,max.iter=max.iter,cov.rate=cov.rate,kertype=kertype,rescale=rescale)
